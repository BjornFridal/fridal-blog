---
title: 'Let''s build a Redux Clone (1/2)'
teaser: '...'
pubDate: null
description: '...'
author: 'Bjørn Fridal'
  ...
  const handleClick = () => {
    ...
    fetch(`https://pokeapi.co/api/v2/pokemon?limit=1&offset=${Math.floor(Math.random() * 1000)}`)
      ...
      .then((json) =>
        ...
        // with the Pokémon's name as the payload
        dispatch({
          type: 'fetched',
          payload: {
...
## What's next? 
In the next blog post we will wrap up our Redux Clone by adding middleware.

You can find the [here source code on Github](https://github.com/BjornFridal/redux-clone/tree/basic).